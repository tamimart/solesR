...
clean_regex_file <- function(regex_file) {
  ...
  if (!all((req_cols) %in% colnames(dat))) {
    stop(
      "Input regex file does not have all required columns: name, type, regex, main_category, sub_category1 and sub_category2"
    )
  }
  ...
  dup_names_tbl <- dat %>%
    group_by(name, type, main_category, sub_category1, sub_category2) %>%
    reframe(n = length(unique(regex)), regex = regex) %>%
    filter(n > 1) %>%
    select(-n)
  ...
  if (n_dup_names > 0) {
    ...
    if (
      !any(unlist(lapply(tb, function(x) any(grepl("test_env", x))))) &&
        interactive()
    ) {
      print(dup_names_tbl)

      proceed_combineRegexes <- menu(
        c("Yes", "No"),
        title = message(
          "Different RegEx that correspond to the same name/concept were found.
  ...
  if (n_dup_regex > 0) {
    ...
    if (
      ...
      proceed_removeDupRegex <- menu(
        c("Yes", "No"),
        title = message(
          "The above entries have duplicate RegEx that correspond to different names/concepts. Should duplicates be removed?
...
add_regex_soles <- function(con, regexfile, master_node, add_node) {
  ...
  for (i in input_types) {
    if (!grepl(ontology_types, i)) {
      message(paste0(
        "New RegEx type '",
        i,
        "' recognized and will be added to the dictionary."
      ))
  ...
  if (!all(is_regex_valid_df$is_valid)) {
    ...
    write.csv(
      is_regex_valid_df,
      file = paste0(
        tools::file_path_sans_ext(basename(regexfile)),
        "_invalid.csv"
      ),
      row.names = F
    )
  ...
  if (!all(valid_regex$name_detected)) {
    ...
    remove_incorrect <- menu(
      c(
        "Return dataframe with results so the RegEx can be inspected before proceeding (recommended)",
    ...
    if (remove_incorrect == "1") {
      write.csv(
        ...
        file = paste0(
          tools::file_path_sans_ext(basename(regexfile)),
          "_name-not-detected.csv"
        ),
        row.names = F
      )
      stop(cat(
        "You can inspect the incorrect entries in the following file: ",
        paste0(
          tools::file_path_sans_ext(basename(regexfile)),
          "_name-not-detected.csv"
        ),
  ...
  try_grepl <- function(pattern, name) {
    tryCatch(
      grepl(pattern, name, ignore.case = T, perl = T),
  ...
  matched <- pico_o %>%
    ...
    mutate(
      match = grepl(
        pattern = paste(correct_regex$regex, collapse = "|"),
        x = name
      ) |
        tolower(name) %in% tolower(correct_regex$name)
    ) %>%
  ...
  existing <- regex_match %>%
    filter(is_new %in% FALSE) %>%
    distinct() %>%
    left_join(pico_o, by = join_by("id_ont" == "regex_id")) %>%
  ...
  existing_entries <- existing %>%
    select(name = name_ont, type = type_ont) %>%
    distinct()
  ...
  if (nrow(regex_to_update) > 0) {
    message(
      ...
      " existing entries with same concept/type were found."
    )
    ...
    purrr::pwalk(
      ...
      function(
        ...
        proceed_update <- menu(
          ...
          title = crayon::blue(paste0(
            "What do you want to update for: ",
            name,
            "?"
          ))
        )

        if (proceed_update %in% c(1, 2)) {
          has_updates <<- TRUE

          cur_regex_id <- current_entry$regex_id[1]

          if (proceed_update == 1) {
            update_data_o <- tibble::tibble(
              name = name,
              type = type,
              main_category = main_category,
              sub_category1 = sub_category1,
              sub_category2 = sub_category2,
              regex_id = cur_regex_id
            )
            ...
            message("Updated all fields: ", name)
          }

          update_data_d <- tibble::tibble(
            id = cur_regex_id,
            regex = regex
          )
  ...
  if (
    ...
    if (n_new > 0) {
      ...
      for (i in ontology_cols) {
        ...
        if (!is.null(new_regex[[i]])) {
          new_regex[[i]] <- ifelse(!is.na(new_regex[[i]]), new_regex[[i]], NA)
        } else {
      ...
      unknown_o_new <- new_types %>%
        ...
        mutate(
          regex_id = seq(from = last_unknown_id + 1, length.out = nrow(.)),
    ...
    if (n_new > 0) {
      ...
      message(
        nrow(pico_d_new),
        ' new regex entries were found.'
      )
      print(tbl_new)
    }

    if (has_updates) {
      message("Updates to existing regex entries were processed.")
    }

    message("These changes will be added to the SOLES OSF dictionary.")
    ...
    if (grepl("y", osf_confirm)) {
      entry_id <- readline(
        "Please enter a domain id for the RegEx you provided (or updated)"
      ) %>%
        na_if("")

      user_id <- readline(
        "Please provide your initials as user identification "
      ) %>%
        na_if("")
      ...
      if (is.na(entry_id) | is.na(user_id)) {
        ...
        write.csv(
          pico_ontology,
          file = "temp/pico_ontology.csv",
          row.names = FALSE
        )
        write.csv(
          pico_dictionary,
          file = "temp/pico_dictionary.csv",
          row.names = FALSE
        )

        # write new additions in separate csv per type (only if n_new > 0)
        if (n_new > 0) {
          ont_split <- split(pico_o_new, pico_o_new$type)
          lapply(ont_split, function(x) {
            write.csv(
              x,
              paste0(
                "temp/",
        ...
        # create log entry - handling with additions and/or updates
        log_list <- list()
        ...
        if (n_new > 0) {
          log_add <- pico_o_new %>%
            group_by(type) %>%
            summarize(n_regex = length(unique(regex_id))) %>%
            mutate(change_type = "addition")
          log_list[[length(log_list) + 1]] <- log_add
        }

        if (has_updates) {
          log_update <- regex_to_update %>%
            group_by(type) %>%
            summarize(n_regex = n()) %>%
            mutate(change_type = "update")
          log_list[[length(log_list) + 1]] <- log_update
        }

        logfile <- bind_rows(log_list) %>%
          mutate(
            domain = entry_id,
            user = user_id,
            timestamp = Sys.time()
...
check_pico <- function(con, master_node) {
  ...
  if (DBI::dbExistsTable(con, "pico_dictionary")) {
    ...
    if (nrow(pico_d_db) < 1) {
      message(
        "The pico_dictionary table is empty. The corresponding OSF table will be written to the database by default."
      )
    ...
    message(
      "No pico_dictionary table was found. The OSF pico_dictionary table will be written to the database by default."
    )
  ...
  if (DBI::dbExistsTable(con, "pico_ontology")) {
    ...
    if (nrow(pico_o_db) < 1) {
      message(
        "The pico_ontology table is empty. The corresponding OSF table will be written to the database by default."
      )
    ...
    message(
      "No pico_ontology table was found. The OSF pico_ontology table will be written to the database by default."
    )
...
create_regex <- function(file = "", path = "") {
  ...
  name_to_regex <- function(string) {
    ...
    for (i in seq_along(words)) {
      ...
      if (grepl("^[[:alpha:]]", words[i], ignore.case = TRUE)) {
        ...
        pattern <- paste0(
          "[",
          toupper(substr(words[i], 1, 1)),
          tolower(substr(words[i], 1, 1)),
          "]",
          substr(words[i], 2, nchar(words[i]))
        )
      } else {
  ...
  with_alternate_names$name_regex <- sapply(
    with_alternate_names$name,
    name_to_regex
  )
  ...
  with_alternate_names$regex_alternate_names <- sapply(
    with_alternate_names$alternate_names,
    convert_alternate_names
  )

  # Concatenates the "name_regex" and "regex_alternate_names" in to 1, separated by "|"
  with_alternate_names$regex <- apply(
    with_alternate_names[, c("name_regex", "regex_alternate_names")],
    1,
